...
* **Single Operator:** Built specifically for the solo engineer & operator. No cumbersome enterprise org trees or multi-tenant ACL overhead.
* **Async Agents (Helper):** Coding agents run asynchronously on dedicated branches and open PRs with automated CI checks. No distracting live chat widgets in the code editor.
...
\`\`\`bash
git clone ssh://git@sourcehub.local:2222/operator/sourcehub-core.git
# or with the first-party CLI
sh repo clone operator/sourcehub-core
\`\`\`
...
export const mockCommits: Commit[] = [
  {
    ...
    authorEmail: 'operator@sourcehub.local',
    date: '18 minutes ago',
    agentRunId: 'run-84f2',
...
This pull request implements fine-grained Personal Access Token (PAT) scope validation across all \`/api/v1/repos/*\` REST endpoints per [§8 of the SourceHub specification](/docs/SOURCEHUB_PLAN.md#8-keys-and-tokens).
...
export const mockWorkflowRuns: WorkflowRun[] = [
  {
    ...
    author: 'Forge Operator (via Helper)',
    duration: '24s',
    createdAt: '18 minutes ago',
    steps: [
      {
...
export const mockAgentRuns: AgentRun[] = [
  {
    ...
    timeline: [
      ...
      },
      {
        ...
        description: 'Stopped at review gate. Waiting for operator to approve and merge.',
        timestamp: '12m ago',
      }
    ],
    logs: [
      ...
      '[15:02:36] API: POST /api/v1/repos/operator/sourcehub-core/pulls -> PR #14 created',
...
export const mockSSHKeys: SSHKey[] = [
  {
    id: 'key-1',
    title: "Operator Workstation (SSH Ed25519)",
...
export const mockTokens: PersonalAccessToken[] = [
  {
    id: 'tok-1',
    name: 'sh-cli-terminal (Workstation)',
