...
export class WorkflowService {
  ...
  async dispatchWorkflow(
    ...
    try {
      const { stdout } = await execAsync('git status -s && git branch --show-current', { cwd: repoPath });
      step1Logs = [
        `Verifying local repository at ${repoPath}...`,
        `Branch: ${branch}`,
        `Working directory status verified.`,
        stdout.trim() ? `Uncommitted files detected:\n${stdout.trim()}` : 'Working tree clean.',
      ];
      steps.push({
    ...
    try {
      const { stdout, stderr } = await execAsync('npm run build', { cwd: repoPath, timeout: 60000 });
